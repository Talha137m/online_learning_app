...
class ExamStart extends StatefulWidget {
  ExamStart({Key? key, required this.examModel}) : super(key: key);
...
class _ExamStartState extends State<ExamStart> {
  ...
  void startTimer() {
    int totalMinutes =
        int.parse(widget.examModel.exam_total_time!.split(":").first);
    setState(() {
  ...
  makeSubmitExam(BuildContext context, isTimeFinish) {
    ...
    for (int i = 0; i < _examQuestionModelList.length; i++) {
      ...
      jsonObject["question_id"] = examQuestionModel.que_id ?? "";
      jsonObject["answer"] = examQuestionModel.selectedIds
          .toString()
          .replaceAll("[", "")
          .replaceAll("]", "");
    ...
    CallApi().post(context, url, map, true).then((response) {
      ...
      if (isTimeFinish) {
        ...
        Route route = MaterialPageRoute(
            builder: (context) => ExamFinished(
                  totalSpendSeconds: totalSpendSeconds,
                  examModel: examModel,
                  examQuestionModelList: _examQuestionModelList,
                ));
