...
class ExamOfflineResult extends StatefulWidget {
  ExamOfflineResult({Key? key, required this.examModel}) : super(key: key);
    ...
    CallApi().post(context, url, map, true).then((response) {
      ...
      setState(() {
        ...
        for (int i = 0; i < _examResultModelList.length; i++) {
          ...
          totalMarks += double.parse(examResultModel.total_marks.toString());
          totalObtain += double.parse(examResultModel.obtain_marks.toString());
        }
