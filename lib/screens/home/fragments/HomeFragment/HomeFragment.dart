...
class _HomeFragmentPageState extends State<HomeFragmentPage> {
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((prefs) {
      ...
      _loginType = sharedPrefs.getInt(BaseURL.KEY_LOGIN_TYPE) ?? 0;
      _updateDetail();
    });
  }
  ...
  _updateDetail() {
    if (sharedPrefs.containsKey("studentData")) {
      var studentData =
          json.decode(sharedPrefs.getString("studentData").toString());
      _studentModel = StudentModel.fromJson(studentData);
    }

    if (sharedPrefs.containsKey("student_list")) {
      final List studentList =
          json.decode(sharedPrefs.getString("student_list").toString());
      _studentList.addAll(
      ...
      body: new Body(
        ...
        myChildClick: () {
          if (sharedPrefs.containsKey("studentList")) {
            final List dataJson =
                json.decode(sharedPrefs.getString("studentList").toString());
            setState(() {
              _studentList.clear();
              _studentList.addAll(
  ...
  makeGetTodayBatches(BuildContext context) {
    ...
    CallApi().post(context, url, map, true).then((response) {
      final List dataJson = json.decode(response.toString());
      sharedPrefs.setString("today_batches", response.toString());
  ...
  makeGetMaster(BuildContext context) {
    ...
    CallApi().post(context, url, map, true).then((response) {
      final Map dataJson = json.decode(response.toString());
